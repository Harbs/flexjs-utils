	...
	{  
		...
		{  
			...
			{  
				...
				out.writeByteAt(int(outPos++),_encodeChars[int(c >>> 18)]);  
				out.writeByteAt(int(outPos++), _encodeChars[int(c >>> 12 & 0x3f)]);  
				out.writeByteAt(int(outPos++), _encodeChars[int(c >>> 6 & 0x3f)]);
				out.writeByteAt(int(outPos++), _encodeChars[int(c & 0x3f)]);
			}
			...
			if (r == 1) //Need two "=" padding
			{
				...
				out.writeByteAt(int(outPos++), _encodeChars[int(c >>> 2)]);
				out.writeByteAt(int(outPos++), _encodeChars[int((c & 0x03) << 4)]);  
			...
			{  
				...
				out.writeByteAt(int(outPos++), _encodeChars[int(c >>> 10)]);
				out.writeByteAt(int(outPos++), _encodeChars[int(c >>> 4 & 0x3f)]);
				out.writeByteAt(int(outPos++), _encodeChars[int((c & 0x0f) << 2)]);  
				out.writeByteAt(int(outPos++), 61);
			}  
		...
		{  
			...
			{  
				//c1  
				c1 = _decodeChars[int(byteString.readByteAt(i++))];  
				if (c1 == -1)  
					break;  
				
				//c2  
				c2 = _decodeChars[int(byteString.readByteAt(i++))];  
				if (c2 == -1)  
					break;  
